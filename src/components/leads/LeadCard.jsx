...
const LeadCard = ({ lead, onEdit, onDelete }) => {
  ...
  return (
    <div className="bg-card border border-slate-100 rounded-2xl p-5 shadow-xs hover:shadow-md hover:border-slate-200 hover:-translate-y-0.5 transition-all duration-300 flex flex-col justify-between gap-4 h-full relative group">
      
      {/* Top Header Section */}
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          {/* Avatar Monogram */}
          <div className="w-10 h-10 rounded-xl bg-slate-100 text-slate-600 font-bold text-sm flex items-center justify-center border border-slate-200/50 group-hover:bg-primary/10 group-hover:text-primary group-hover:border-primary/20 transition-colors duration-300">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {lead.value > 0 && (
          <span className="inline-flex items-center px-2 py-0.5 bg-success/10 text-success border border-success/15 rounded-md text-[10px] font-bold">
      ...
      <div className="space-y-2 pt-3 border-t border-slate-100 text-xs font-semibold text-text-gray">
        {/* Email */}
        <a
