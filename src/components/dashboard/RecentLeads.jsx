...
const STATUS_BADGE_STYLES = {
  New: 'bg-primary/5 dark:bg-primary/10 text-primary border-primary/15 dark:border-primary/25',
  Contacted: 'bg-accent/10 dark:bg-accent/20 text-accent border-accent/20 dark:border-accent/30',
  Qualified: 'bg-secondary/10 dark:bg-secondary/20 text-secondary border-secondary/20 dark:border-secondary/30',
  Proposal: 'bg-secondary/10 dark:bg-secondary/20 text-secondary border-secondary/20 dark:border-secondary/30',
  Negotiation: 'bg-accent/10 dark:bg-accent/20 text-accent border-accent/20 dark:border-accent/30',
  'Closed Won': 'bg-success/10 dark:bg-success/20 text-success border-success/20 dark:border-success/30',
  'Closed Lost': 'bg-danger/10 dark:bg-danger/20 text-danger border-danger/20 dark:border-danger/30',
};
    ...
    <div className="bg-card border border-border rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-border text-xs font-semibold text-text-gray uppercase tracking-wider">
              ...
              recentFiveLeads.map((lead) => (
                ...
                >
                  ...
                  <td className="py-3.5 pl-1 flex items-center gap-3">
                    <div className="w-8.5 h-8.5 rounded-full bg-background flex items-center justify-center text-xs font-bold text-text-gray border border-border group-hover:bg-primary/10 group-hover:text-primary group-hover:border-primary/20 transition-colors duration-200">
                  ...
                  {/* Company */}
                  <td className="py-3.5 text-sm text-text-gray font-medium hidden md:table-cell">
                    {lead.company}
                  </td>
                  ...
                  <td className="py-3.5">
                    <span 
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${
                        STATUS_BADGE_STYLES[lead.status] || 'bg-background text-text-gray border-border'
                      }`}
                    >
                      {lead.status}
                    </span>
                  ...
                  {/* Date Added */}
                  <td className="py-3.5 pr-1 text-right text-xs text-text-gray font-semibold hidden md:table-cell">
